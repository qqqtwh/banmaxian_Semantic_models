...
def parse_opt():
    ...
    parse.add_argument('--height', type=int, default=512, help='输入模型的图片高度')
    parse.add_argument('--width', type=int, default=512, help='输入模型的图片宽度')
...
if __name__ == '__main__':
    ...
    for i,jpg_name in enumerate(imgs_path):
        ...
        img = np.array(img) / 255  # (512,512,3) [0,1]
        imgs_test.append(img)
        imgs_init.append(img_init)
        jpg_names.append(jpg_name)

    imgs_test = np.array(imgs_test)  # (-1,512,512,3)
    # 3.模型创建
    weight_path = 'weights/deeplabv3_weight/'
    model = build_deeplabv3(opt.n_classes,opt.height,opt.width)
    model.load_weights(os.path.join(weight_path, 'last.h5'))
    # 4.模型预测语义分类结果
    prs = model.predict(imgs_test)  # (-1, 512*512, 2)
    # 结果 reshape
    prs = prs.reshape(-1, opt.height,opt.width, opt.n_classes)  # (-1, 512, 512, 2)
    ...
    imgs_seg = np.zeros((len(prs), opt.height,opt.width, 3)) # (-1,512,512,3)
