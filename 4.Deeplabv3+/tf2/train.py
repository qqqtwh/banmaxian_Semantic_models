...
def parse_opt():
    ...
    parse.add_argument('--height',type=int,default=512,help='图片高度')
    parse.add_argument('--width',type=int,default=512,help='图片宽度')
...
def get_data_from_file(opt):
    ...
    for i in range(len(lines)):
        ...
        real_img = np.array(real_img)/255   # (512,512,3) [0,1]
        X.append(real_img)
        # 读取标签图像，3通道，每个通道的数据都一样，每个像素点就是对应的类别，0表示背景
        label_img = Image.open(os.path.join(datasets_path, 'png', label_name))
        label_img = label_img.resize((height, width))
        label_img = np.array(label_img) # (512,512,3)
        # 根据标签图像来创建训练标签数据，n类对应的 seg_labels 就有n个通道
        # 此时 seg_labels 每个通道的都值为 0
        seg_labels = np.zeros((height,width,n_classes))  # (512,512,2)
        # 第0通道表示第0类
        # 第1通道表示第1类
        # .....
        ...
        seg_labels = np.reshape(seg_labels,(-1,n_classes))  # (512*512,2)
        Y.append(seg_labels)
